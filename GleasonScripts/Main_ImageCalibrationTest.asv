% Name: Main_ImageCalibrationTest
% Authors: Devin Thompson
% Date: 8/7/2023
% Description: Processes the image data to 
...
% marker
testImage = imread(strcat(imagePath, "\","C02-01-08022023_Neutral.jpg")); % Edit this each time
...
level = 0.95; % Adjust this to change the threshold level
...
%"Method","TwoStage");
[centers, radii, metrics] = imfindcircles(imBW, [5 20],"Sensitivity",0.8);
viscircles(centers, radii)
%figure; imshow(imBW); viscircles(centers, radii)
for i =1:length(centers)
    labelString = num2str(i);
    text(centers(i,1)+15, centers(i,2)+15, labelString,"Color",'g')
end
...
worldDiameter1 = sqrt((worldSides2(:,1)-worldSides1(:,1)).^2 + (worldSides2(:,2)-worldSides1(:,2)).^2)

%% Convert pixel coordinates to global coordinates
% imfindcirlces doesn't get the diameter quite right, use imtool, use the
% distance tool and export manually
%imtool(im)
...
pixelPoints = [point12; point22];
worldPoints = img2world2d(pixelPoints,camExtrinsics,camIntrinsics);
% Compute the distance between them
wordDiameter4 = sqrt((worldPoints(1,1)-worldPoints(2,1)).^2 + (worldPoints(1,2)-worldPoints(2,2)).^2)

pixelPoints = [point13; point23];
worldPoints = img2world2d(pixelPoints,camExtrinsics,camIntrinsics);
% Compute the distance between them
wordDiameter5 = sqrt((worldPoints(1,1)-worldPoints(2,1)).^2 + (worldPoints(1,2)-worldPoints(2,2)).^2)

pixelPoints = [point14; point24];
worldPoints = img2world2d(pixelPoints,camExtrinsics,camIntrinsics);
% Compute the distance between them
wordDiameter6 = sqrt((worldPoints(1,1)-worldPoints(2,1)).^2 + (worldPoints(1,2)-worldPoints(2,2)).^2)
